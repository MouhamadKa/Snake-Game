...
class Game:
    ...
    def handle_eat(self):
        self.score += 1
        self.food.position = self.food.create(self.snake.snake_body)
 
    def did_collide(self):
        return self.snake.head[0] < 0 or self.snake.head[0] >= self.width \
               or self.snake.head[1] < 0 or self.snake.head[1] >= self.height \
    ...
    def handle_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                self.handle_keypress_events(event.key)
                
    def handle_keypress_events(self, key):
